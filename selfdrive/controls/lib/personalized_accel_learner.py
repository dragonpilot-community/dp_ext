...
A_MAX_BP = [0, 0.5, 1, 1.5, 2, 3, 4, 5, 6, 8, 10, 12, 15, 20, 25, 30, 32.5, 35, 37.5, 40]
SAMPLES = [10, 10, 10, 10, 10, 10, 15, 15, 15, 15, 20, 20, 30, 30, 30, 30, 30, 30, 30]
...
class PersonalizedAccelLearner:
    def __init__(self):
        ...
        self.profile_size = len(A_MAX_BP)

        self.MAX_VALS = np.maximum(0, np.interp(A_MAX_BP, A_CRUISE_MAX_BP, A_CRUISE_MAX_VALS)).tolist()
    ...
    def _initialize_variable_samples(self):
        return [10, 10, 10, 10, 10, 10, 15, 15, 15, 15, 20, 20, 20, 20, 25, 25, 25, 25, 25]
    ...
    def update(self, car_state, lead_present):
        if (not lead_present and
            ...
            if speed_index < len(SAMPLES):
                if len(self.speed_range_samples[speed_index]) >= SAMPLES[speed_index]:
    ...
    def _get_speed_index(self, speed):
        index = bisect.bisect_right(A_MAX_BP, speed) - 1
        return max(0, min(index, len(A_MAX_BP) - 1))
    ...
    def get_max_accel(self, v_ego, STOCK_BP, STOCK_VALS):
        if self.enabled:
            val = np.interp(v_ego, A_MAX_BP, self.A_CRUISE_MAX_VALS)
        else:
    ...
    def load_model(self):
        ...
        if params_val is not None:
            ...
            self.A_CRUISE_MAX_VALS = data.get('vals', self.A_CRUISE_MAX_VALS)
